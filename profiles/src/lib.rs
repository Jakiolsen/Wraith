...
impl C2Profile {
        ...
        for entry in entries.flatten() {
            ...
            if path.extension().and_then(|e| e.to_str()) == Some("toml") {
                match Self::from_file(&path) {
                    ...
                    Err(err) => eprintln!("WARN: skipping invalid profile {}: {err}", path.display()),
                }
            }
        }
        Ok(profiles)
    ...
    pub fn base_url(&self) -> String {
        format!("{}://{}:{}", self.transport.protocol.scheme(), self.transport.host, self.transport.port)
    }

    pub fn checkin_url(&self) -> String { format!("{}{}", self.base_url(), self.http.checkin_uri) }
    pub fn result_url(&self)  -> String { format!("{}{}", self.base_url(), self.http.result_uri)  }
}

impl Protocol {
    pub fn scheme(&self) -> &'static str {
        match self { Self::Http => "http", Self::Https => "https" }
    }
}

fn default_sleep_ms()             -> u64    { 5_000 }
fn default_jitter_pct()           -> u64    { 20 }
fn default_protocol()             -> Protocol { Protocol::Https }
fn default_port_https()           -> u16    { 443 }
fn default_checkin_uri()          -> String { "/api/v1/update".to_owned() }
fn default_result_uri()           -> String { "/api/v1/result".to_owned() }
fn default_internal_checkin_uri() -> String { "/implant/checkin".to_owned() }
fn default_internal_result_uri()  -> String { "/implant/result".to_owned() }
