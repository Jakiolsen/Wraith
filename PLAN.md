...
**Checkpoint:** ✅ `make docker-secrets && make docker-up && make docker-provision && make client` → login works → implant beacons in → operator sees session → `sysinfo` output appears.
