...
| Crate | Status |
|---|---|
| `server` | ✅ HTTP + gRPC mTLS + PostgreSQL + CA/CRL + Docker |
| `implant` | ✅ Beacon loop, jitter, 5 common modules |
| `client` | ✅ mTLS connect screen, sessions table, task console, dark theme |
...
- Bearer token auth replaced by mTLS in Step 2

**1.8 Operator client** ✅
- Connect screen (cert/key/CA paths) → sessions table (auto-refresh every 5s) → task console
- Module selector, args input, Dispatch, task history with status colour coding
- Dark theme; Disconnect button wipes auth state
...
- CA private key stored encrypted in Postgres (AES-256-GCM, Argon2id KDF with explicit params m=19 MB/t=2/p=1)
- Server's own TLS cert (1-year validity) also generated by the CA at first start, stored encrypted in DB
- CA dropped from process RAM immediately after TLS is configured — not resident during serve mode
- `ca_state` table: `ca_cert_pem`, encrypted `ca_key`, encrypted `server_key`, `kdf_salt`, `server_cert_pem`
- `CA_PASSPHRASE` required at startup; `--dev-no-passphrase` flag for development only
- gRPC defaults to `127.0.0.1:50051`; Docker compose overrides to `0.0.0.0:50051` (host binding restricts externally)

**2.2 Operator cert provisioning** ✅
- CLI subcommand: `wraith-server provision-operator --username <n> --role <r> [--out-dir <dir>]`
- Generates a leaf cert signed by the CA with CN=username, OU=role, `BasicConstraints::Constrained(0)`
- `--out-dir` writes `<username>.cert.pem` (world-readable), `<username>.key.pem` (0600), `ca.cert.pem`
- Public cert + serial stored in `operator_certs` table; private key never persisted server-side
- Re-provisioning the same username automatically revokes the old serial into the CRL before issuing the new cert
- `docker-provision` Makefile target writes cert files to `./certs/` via bind mount

**2.3 mTLS on gRPC** ✅
- `ServerTlsConfig::new().identity(server_cert).client_ca_root(ca_cert)` — requires valid client cert
- Peer cert extracted from `TlsConnectInfo<TcpConnectInfo>` on every gRPC call
- CN used as operator username, OU as role; no passwords or tokens anywhere
- CRL table in Postgres: serial checked against in-memory `HashSet` (refreshed every 60s, poison-safe)
- Redirector token comparison is constant-time via `subtle::ConstantTimeEq`
- Bearer token auth and `Login` RPC completely removed from server, client, and shared crate
...
- `revoke-operator --username <n>`: inserts cert serial into `crl` table; takes effect on next connection
- CRL refreshed from DB every 60s in a background task — zero downtime on revocation
- Certs valid for 7 days; `docker-secrets` prompts for CA passphrase alongside postgres password

**2.6 Implant HTTP hardening** ✅
- 1 MB body limit on all implant HTTP endpoints (axum `DefaultBodyLimit`)
- Session ID validated against DB on every checkin: unknown IDs (spoofed or DB-wiped) get a fresh UUID
- Task results cross-referenced by `(task_id, session_id)` — an implant cannot write results into another session's tasks
- All timestamps stored in UTC

**Checkpoint:** ✅ Operator connects with PEM cert/key files; server rejects connections without a valid CA-signed cert; revoking a cert takes effect within 60 seconds; oversized implant payloads are rejected with 413.
